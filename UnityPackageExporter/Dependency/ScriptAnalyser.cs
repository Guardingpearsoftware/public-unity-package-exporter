...
{
    /// <summary>
    /// Analyses C# scripts to find dependencies between classes using Roslyn.
    /// </summary>
    class ScriptAnalyser : IDisposable
    {
        ...
        /// <summary>
        /// Roslyn workspace for the project.
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the ScriptAnalyser class.
        /// Sets up the Roslyn workspace and project.
        /// </summary>
        /// <param name="projectPath">Project root path.</param>
        /// <param name="logger">Logger.</param>
        public ScriptAnalyser(string projectPath, ILogger logger)
        {
            _logger = logger;
        ...
        /// <param name="source">Source code content.</param>
        public void AddSource(string name, string source)
        {
            _logger.LogTrace("Adding source {0}", name);
        ...
        public async Task<bool> AddFileAsync(string file)
        {
            ...
            if (!File.Exists(file))
            {
                _logger.LogError("File {0} does not exist", file);
                return false;
            }
            ...
            {
                _logger.LogError("File {0} source is empty", file);
                return false;
            }
            ...
            if (src == null)
            {
                _logger.LogError("File {0} source is invalid", file);
                return false;
            }
            ...
            if (src == null)
            {
                _logger.LogError("File {0} document is invalid", file);
                return false;
            }
        ...
        /// <summary>Finds the shallow list of dependencies for a single file</summary>
        /// <param name="file">File to check.</param>
        /// <returns>Array of dependent file paths.</returns>
        ...
        /// <summary>Builds the internal dependency map by analysing all documents in the project</summary>
        public async Task BuidDependencyMap()
        {
            _logger.LogInformation("Building Dependency Map");
            ...
            _logger.LogTrace("Finished building map. Took {0}ms", stopwatch.ElapsedMilliseconds);
        }

        /// <summary>
        /// Analyses a single document to find what other files refer to symbols declared in it.
        /// </summary>
        /// <param name="sourceDocument">The document declaring symbols.</param>
        /// <param name="mapping">Thread-safe dictionary to update with found dependencies.</param>
        private async Task FindSourceDependents(Document sourceDocument, ConcurrentDictionary<string, ConcurrentBag<string>> mapping)
        {
            ...
            _logger.LogTrace("Scanning references of {0}", sourceFile);
        ...
        /// <summary>
        /// Helper to run a task in parallel with a limited degree of parallelism.
        /// </summary>
